...
constants/
  ...
  sessionsApi.ts   # Акты инвентаризации: адаптер под бэкенд + оффлайн
  offline.ts       # Оффлайн: копии данных, очередь, досылка
  offlineCore.ts   # Правила очереди без RN — тесты в offlineCore.test.mjs
...
```bash
npm test   # логика оффлайн-очереди (constants/offlineCore.ts), node --test
```
...
Сканирование актов работает без сети. Код: `constants/offline.ts` (хранилище, очередь, досылка) и `constants/offlineCore.ts` (чистая логика, покрыта тестами).
