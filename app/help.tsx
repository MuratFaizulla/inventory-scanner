...
function StatusActions() {
  const rows: { emoji: string; color: string; label: string; action: string }[] = [
    ...
    { emoji: '⚠️', color: Colors.warn, label: 'Не на месте',
      action: 'Нажмите «Изменить» и укажите фактический кабинет и сотрудника; если сотрудника нет в списке — впишите вручную. Это попадёт в примечание акта; в 1С исправляется вручную.' },
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.card}>
          ...
          <Steps color={Colors.accent2} steps={[
            ...
            { t: 'Завершить',
              d: 'Меню «⋮» → «Завершить». Все непроверенные ОС автоматически получают статус «Не найдено» — завершайте только когда обошли всё. Нужна связь: сначала на сервер уйдут сканы, сделанные без Wi-Fi.' },
            { t: 'Посмотреть итог',
              d: 'Сразу после завершения откроется итог акта: что не нашли, излишки, смены кабинета и сотрудника — и что с каждым сделать в 1С. Позже — кнопка «Итог» на карточке акта. Пока акт идёт, в деталях есть предварительный итог.' },
          ]} />
        </View>
        ...
        {/* ── 10. Без Wi-Fi ── */}
        <View style={s.card}>
          <SectionHead icon="wifi-off" color={Colors.warn} title="Сканирование без Wi-Fi" roles="админ и руководство" />
          <Steps steps={[
            { t: 'Пока есть связь — откройте «Акты»',
              d: 'Запущенные акты сохранятся на телефон сами. Открытый сканер тоже сохраняет свой акт.' },
            { t: 'Сканируйте как обычно',
              d: 'Без связи результат считается по копии акта, а скан ложится в очередь — сверху плашка «Нет связи · N операций».' },
            { t: 'Код не из акта',
              d: '«Нет в акте — проверим при отправке»: излишек это или ОС нет в базе, решит сервер.' },
            { t: 'Вернулись в Wi-Fi',
              d: 'Очередь уйдёт сама (или кнопка «Отправить»). В акт попадёт время скана, а не отправки.' },
            { t: '«Сервер не принял»',
              d: 'Например, акт успели завершить. Нажмите на плашку — там список кодов, проверьте их в деталях акта.' },
          ]} />
          <Note text="Приложение без связи тоже открывается: войдите тем же логином и паролем. Не выходите из аккаунта, пока очередь не отправлена, — сканы не пропадут, но уйдут только после вашего входа." />
        </View>
